...
public class AINavPath : MonoBehaviour
{
	...
	public AINavNode GetNextAINavNode(AINavNode node)
	{
		...
		if (pathType == ePathType.Djikstra || pathType == ePathType.AStar) return GetNextPathAINavNode(node);
		return null;
	}
	...
	private void GeneratePath(AINavNode startNode, AINavNode endNode)
	{
		...
		if (pathType == ePathType.Djikstra) AINavDijkstra.Generate(startNode, endNode, ref path);
